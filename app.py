...
def batch_page():
    ...
    if "result_csv_bytes" not in st.session_state:
    ...
    with right:
        ...
        if uploaded_file is not None:
            try:
                ...
                if input_type == "XYZ":
                    ...
                    y_col = df.columns.get_loc(st.selectbox("y column", options=df.columns, key="y_col"))
                    ...
                    if llh_type == "Decimal Degrees":
                        c1, c2 = st.columns(2)
                        with c1:
                            lat_deg_col = df.columns.get_loc(st.selectbox("Latitude column", options=df.columns, key="lat_deg_col"))
                        with c2:
                            lon_deg_col = df.columns.get_loc(st.selectbox("Longitude column", options=df.columns, key="lon_deg_col"))

                        h_col = df.columns.get_loc(st.selectbox("Height column", options=df.columns, key="h_col"))

                    elif llh_type == "Degrees Minutes Seconds":
                        c1, c2, c3 = st.columns(3)
                        with c1:
                            lat_deg_col = df.columns.get_loc(st.selectbox("Latitude degrees column", options=df.columns, key="lat_deg_col"))
                            lon_deg_col = df.columns.get_loc(st.selectbox("Longitude degrees column", options=df.columns, key="lon_deg_col"))
                        with c2:
                            lat_min_col = df.columns.get_loc(st.selectbox("Minutes column", options=df.columns, key="lat_min_col"))
                            lon_min_col = df.columns.get_loc(st.selectbox("Minutes column", options=df.columns, key="lon_min_col"))
                        with c3:
                            lat_sec_col = df.columns.get_loc(st.selectbox("Seconds column", options=df.columns, key="lat_sec_col"))
                            lon_sec_col = df.columns.get_loc(st.selectbox("Seconds column", options=df.columns, key="lon_sec_col"))
                        
                        h_col = df.columns.get_loc(st.selectbox("Height column", options=df.columns, key="h_col"))
                    elif llh_type == "Degrees Decimal Minutes":
                        c1, c2 = st.columns(2)
                        with c1:
                            lat_deg_col = df.columns.get_loc(st.selectbox("Latitude degrees column", options=df.columns, key="lat_deg_col"))
                            lon_deg_col = df.columns.get_loc(st.selectbox("Longitude degrees column", options=df.columns, key="lon_deg_col"))
                        with c2:
                            lat_min_col = df.columns.get_loc(st.selectbox("Latitude decimal minutes column", options=df.columns, key="lat_min_col"))
                            lon_min_col = df.columns.get_loc(st.selectbox("Longitude decimal minutes column", options=df.columns, key="lon_min_col"))
                        h_col = df.columns.get_loc(st.selectbox("Height column", options=df.columns, key="h_col"))
                elif input_type == "ENU":
                    c1, c2, c3 = st.columns(3)
                    with c1:
                        e_col = df.columns.get_loc(st.selectbox("Easting column", options=df.columns, key="e_col"))
                    with c2:
                        n_col = df.columns.get_loc(st.selectbox("Northing column", options=df.columns, key="n_col"))
                    with c3:
                        u_col = df.columns.get_loc(st.selectbox("Height column", options=df.columns, key="u_col"))
                    zone_col = df.columns.get_loc(st.selectbox("Zone column", options=df.columns, key="zone_col"))
            except Exception as e:
        ...
        if not st.session_state.has_result:
            ...
            if errors:
                ...
                if input_type == "XYZ":
                    csv_params = ft.CSVCoordinateMapping(x=x_col, y=y_col, z=z_col, coord_type="xyz")
                elif input_type == "LLH":
                    if llh_type == "Decimal Degrees":
                        csv_params = ft.CSVCoordinateMapping(lat_deg=lat_deg_col, lon_deg=lon_deg_col, el_height=h_col, coord_type="llh", angle_format="dd")
                    elif llh_type == "Degrees Minutes Seconds":
                        csv_params = ft.CSVCoordinateMapping(lat_deg=lat_deg_col, lat_min=lat_min_col, lat_sec=lat_sec_col, lon_deg=lon_deg_col, lon_min=lon_min_col, lon_sec=lon_sec_col, el_height=h_col, coord_type="llh", angle_format="dms")
                    elif llh_type == "Degrees Decimal Minutes":
                        csv_params = ft.CSVCoordinateMapping(lat_deg=lat_deg_col, lat_min=lat_min_col, lon_deg=lon_deg_col, lon_min=lon_min_col, el_height=h_col, coord_type="llh", angle_format="ddm")
                elif input_type == "ENU":
                    csv_params = ft.CSVCoordinateMapping(east=e_col, north=n_col, el_height=u_col, zone=zone_col, coord_type="enu")

                # Transformation for map
                uploaded_file.seek(0)  # reset file pointer to start
                map_csv = ft.csv_transformation(uploaded_file, csv_params, trans_params_map)
                uploaded_file.seek(0)  # reset file pointer to start
                ...
                if result_csv:
                    ...
                    with open(result_csv, "rb") as f:
                        st.download_button(
                                ...
                                data=f.read(),
                                file_name=Path(result_csv).name,
                                mime="text/csv"
                            )
                ...
                # Build a dataframe for ALL transformed points
                df = pd.DataFrame({
                    "lat": map_coords["lat_transformed"].astype(float),
                    "lon": map_coords["lon_transformed"].astype(float),
...
pages = [
    st.Page(point_page, title="Single Point"),
